...
from uploadOcr import process_file_async
import numpy as np
import cv2
...
while(True):
    ...
    if moving :
        if (count < lowerbound):
            ...
            thresh = cv2.adaptiveThreshold(step1,255,
                cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY,11,2)
            cv2.imshow(tag + " thresh", thresh)
            ...
            for c in cnts:
                ...
                if size >1000 and size < 5000:
                    ...
                    if 5*h > w and 3*h < w: 
                        ...
                        process_file_async(chunkfilename)
                        chunkCounter+=1
                    else:
                        cv2.rectangle(frame3, (x, y), (x + w, y + h), (0, 255, 255), 2)
